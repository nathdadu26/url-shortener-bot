...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Agar user ne bot block kiya hai — silently ignore karo
    if isinstance(error, Forbidden):
        if update and hasattr(update, "effective_user") and update.effective_user:
            print(f"[IGNORED] User {update.effective_user.id} ne bot block kiya hai.")
        return

    # Baaki errors ko log karo
    print(f"[ERROR] Update: {update} | Error: {error}")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.photo:
        await message.reply_photo(photo=message.photo[-1].file_id, caption=reply)
    elif message.video:
        await message.reply_video(video=message.video.file_id, caption=reply)
    elif message.document:
        await message.reply_document(document=message.document.file_id, caption=reply)
    elif message.animation:
        await message.reply_animation(animation=message.animation.file_id, caption=reply)
    else:
...
def main():
    ...
    # Global error handler — bot crash nahi hoga
